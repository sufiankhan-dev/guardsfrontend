...
const VoucherCard = ({ row }) => {
    ...
    return (
      <div className="flex items-center">
        <div className="bg-blue-400 text-white rounded-lg p-4 flex-1">
          <h3 className="text-lg text-white font-bold mb-2">Voucher Card</h3>
          <div className="mb-2">
            <span className="font-semibold mr-2">Code:</span>
            <span>{row.original.code}</span>
            <button
              className="ml-2 bg-black-500 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded focus:outline-none"
          ...
          <p className="text-yellow-400">(This is your voucher information.)</p>
        </div>
      </div>
    );
  };
...
const Discounttable = () => {
  ...
  const fetchData = async (pageIndex, pageSize) => {
    try {
      ...
      setUserData(response.data.discounts);
      console.log(response,"araha");
      setPageCount(Math.ceil(response.data.total / pageSize)); // Assuming 'total' is the total number of users
    } catch (error) {
      console.log(error);
    }
  };
    ...
    <>
      <Card noborder>
        <div className="md:flex pb-6 items-center">
          <h6 className="flex-1 md:mb-0 mb-3">Discount</h6>
