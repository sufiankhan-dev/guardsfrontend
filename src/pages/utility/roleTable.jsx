...
const RolePage = () => {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_BASE_URL}/${user.type}/role/delete-role/${id}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <Dropdown
          ...
          label={<span className="text-xl text-center block w-full"><Icon icon="heroicons-outline:dots-vertical" /></span>}
        >
      ...
      hooks.visibleColumns.push((columns) => [
        {
          ...
          Cell: ({ row }) => <div><IndeterminateCheckbox {...row.getToggleRowSelectedProps()} /></div>,
        },
        ...columns,
      ]);
    }
